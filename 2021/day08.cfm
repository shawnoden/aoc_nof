<cfscript>
    ...
    inp = inFile.reduce( (prev,nxt) => { return prev.append(nxt.listToArray(" | ", false,true)[2])},[] ) ;
    //inp = inFile
    ...
    function solvePart1( inp ) {
        var retval = 0 ;
        inp2 = inp.reduce( (prev,nxt) => { return prev.append( nxt.map( (n)=>n.listtoArray(" ") ) ) }, [] ) ;
        inp2.filter( (el)=> (el.len()==2 || el.len()==3 ||el.len()==4 || el.len()==7)?:false )
retval = inp2
        return retval
       //// ANSWER : 
    }
